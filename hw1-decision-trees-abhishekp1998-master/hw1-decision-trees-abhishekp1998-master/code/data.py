...
def load_data(data_path):
    ...
    with open(data_path) as csv_file:
        ...
        for row in csv_reader:
            if line_count == 0:
                attribute_names = row[0:len(row) - 1]
                line_count += 1
            else:
                if features:
                    features.append(list(map(float, row[0:len(row) - 1])))
                    targets.append(float(row[len(row) - 1]))
                else:
                    features = [list(map(float, row[0:len(row) - 1]))]
                    targets = [float(row[len(row) - 1])]
    return np.asarray(features), np.asarray(targets), attribute_names
def train_test_split(features, targets, fraction):
    ...
    test_targets = None 
    n = int(features.shape[0]*fraction)
    x = int(targets.shape[0]*fraction)

    train_features = features[0:n]
    test_features = features[features.shape[0] - n: features.shape[0]]
    train_targets = targets[0:x]
    test_targets = targets[targets.shape[0]- x: targets.shape[0]]
